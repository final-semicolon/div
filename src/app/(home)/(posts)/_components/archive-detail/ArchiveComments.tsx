        ...
        <div key={comment.id} className="w-full flex flex-col ">
          ...
          >
            ...
            {editingState[comment.id] ? (
              ...
              <p className="text-body1 font-regular whitespace-pre-wrap break-words">{filterSlang(comment.comment)}</p>
            ) : (
              <div>
                ...
                {comment.comment.length >= 370 && (
                  <button className="text-subtitle2 font-bold text-neutral-700" onClick={() => setCommentLength(true)}>
                    ...더보기
                  </button>
                )}
              </div>
          ...
          {inputReplyToggle[comment.id] ? (
            <ArchiveReplyInput comment_id={comment.id} toggle={handleInputReplyToggle} count={comment.reply[0].count} />
          ) : null}
