    ...
    <>
      ...
      <Mobile>
        <div className="flex justify-center items-center mx-5">
          <button
            onClick={() => setStatus('waiting')}
            className={`flex justify-center items-center flex-grow py-4 ${
              status === 'waiting' ? 'bg-sub-50 text-neutral-800' : 'bg-white text-neutral-300'
            } rounded-tl-3xl rounded-tr-3xl transition-colors duration-300`}
          >
            <p className="text-subtitle3 font-bold"> 채택을 기다리는 질문글</p>
          </button>
          <button
            onClick={() => setStatus('selected')}
            className={`flex justify-center items-center flex-grow py-4 ${
          ...
          >
            <p className="text-subtitle3 font-bold"> 채택을 완료한 질문글</p>
          </button>
        </div>
      </Mobile>
    </>
